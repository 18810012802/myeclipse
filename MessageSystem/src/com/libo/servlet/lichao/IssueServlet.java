...
public class IssueServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int rows=ser.addinfo(name, number, content, endDate,oid, state);
			int fid=ser.getFileId();
			ser.addFile(fname, pa, ftype, mid,fid);
			if (rows > 0) {
				out.print("<script>alert('发布成功！');location.href='ShowServlet'</script>");
			} else {
				out.print("<script>alert('发布失败！');location.href='ShowServlet'</script>");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
