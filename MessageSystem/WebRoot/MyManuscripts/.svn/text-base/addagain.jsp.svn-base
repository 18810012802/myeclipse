...
<form action="MyFileUpdaServlet?chose=chose&mid=${fil.id }&userid=<%=session.getAttribute("org_id") %>" method="post" enctype="multipart/form-data" class="definewidth m20" >
<input type="hidden" name="maid" value="${fil.maId }" />
<table class="table table-bordered table-hover definewidth m10">
  <tr>
        <td width="10%" class="tableleft">所属期刊</td>
        <td>
			<select name="namenu" value="${fil.number }">
			<c:forEach items="${mmas }" var="mmas">
				<option value="${fil.number }">${mmas.name }${mmas.number }期</option>
				</c:forEach>
			</select>
		</td>
    </tr>   
   <tr>
        ...
        <td><input type="text" name="username" value="${ore }"  readonly /></td>
       
    </tr>
    <tr>
        <td width="10%" class="tableleft">标题</td>
        <td><input type="text" name="title" value="${fil.title }" /></td>
    </tr>
	
	<tr>
	<c:forEach items="${ore }" var="o">
        <td width="10%" class="tableleft">作者</td>
        <td><input type="text" name="author" value="${ore }" /></td>
        </c:forEach>
    </tr>
	<tr>
        <td width="10%" class="tableleft">来源</td>
        <td><input type="text" name="from" value="${fil.from }" /></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">类别</td>
        <td>
		<select name="type" value="${fil.type }">
	...
	<tr>
        ...
        <td><select name="filetype" value="${fil.filetype }">
