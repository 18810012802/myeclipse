$(function() {
	obj = {
		...
		search : function() {
			$('#table').datagrid('load', {
				train_no : $.trim($('input[name="trainNo"]').val()),
			});
		},
		add : function() {
		...
		update : function() {
			...
			if (rows.length == 1) {
				...
				$.messager.alert('警告', '修改必须且只能有一行', 'warnning');
			}
		},
		del : function() {
			var rows = $('#table').datagrid('getSelections');
			...
			.datagrid(
					{
						...
						onAfterEdit : function(rowIndex, rowData, changes) {
							...
							$('#save,#redo').hide();
							var inserted = $('#table').datagrid('getChanges',
									'inserted');
							var updated = $('#table').datagrid('getChanges',
									'updated');
							var type = "";
							var tip = "";
						...
						columns : [ [
						{
							...
							formatter:function(value,row,index){
								return '<a href="TrainServlet?t=look&train_no='+value+'">'+value+'</a>';
							},
						}, {
							field : 'station',
							title : '发车 -- 到达',
